<?php
$page_title = 'Previous Executive Committee Documents';
...
try {
    $stmt_count = $pdo->prepare("SELECT COUNT(*) FROM `committee_documents` WHERE `doc_type` = ?");
    $stmt_count->execute([$doc_type]);
    $total_rows = $stmt_count->fetchColumn();
    $total_pages = max(1, ceil($total_rows / $limit));
    
    $stmt = $pdo->prepare("SELECT * FROM `committee_documents` WHERE `doc_type` = :doc_type ORDER BY `year` DESC, `created_at` DESC LIMIT :limit OFFSET :offset");
...
if ($edit_id > 0) {
    try {
        $stmt_edit = $pdo->prepare("SELECT * FROM `committee_documents` WHERE `id` = ? AND `doc_type` = ?");
    ...
    <div class="recent-card">
        <div class="recent-header">
            <h3 class="recent-title">
                ...
                <?php echo $edit_id > 0 ? 'Edit Document details' : 'Upload Previous Committee PDF'; ?>
            </h3>
        </div>
        <div style="padding: 2rem;">
            <form id="document-form" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group" style="margin-bottom: 1.25rem;">
                    ...
                    <input type="text" id="doc_title" name="title" class="form-control" placeholder="e.g. Executive Committee List 2024-25" value="<?php echo htmlspecialchars($edit_title); ?>" required>
                </div>
                
                <!-- Target Year -->
                <div class="form-group" style="margin-bottom: 1.25rem;">
                    <label class="form-label" for="doc_year">Year <span style="color: var(--red);">*</span></label>
                    <input type="text" id="doc_year" name="year" class="form-control" placeholder="e.g. 2024-2025" value="<?php echo htmlspecialchars($edit_year); ?>" required>
                </div>

                <!-- PDF File Upload Dropzone -->
                <div class="form-group" style="margin-bottom: 1.5rem;">
                    <label class="form-label">Upload PDF Document <?php echo $edit_id > 0 ? '<small style="color: var(--gray); font-weight: normal;">(Leave empty to keep current PDF)</small>' : '<span style="color: var(--red);Required">*</span>'; ?></label>
                ...
                <div style="display: flex; gap: 0.6rem;">
                    <?php if ($edit_id > 0): ?>
                        <a href="committee_previous.php" class="btn btn-cancel" style="flex: 1; text-align: center; text-decoration: none; display: flex; align-items: center; justify-content: center; height: 42px; border-radius: 6px;">Cancel</a>
                    <?php endif; ?>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center; padding: 1rem 1.5rem;">
                                        <div style="display: inline-flex; gap: 0.4rem;">
                                            <a href="committee_previous.php?edit=<?php echo $doc['id']; ?>&page=<?php echo $page; ?>" class="btn-action btn-edit" title="Edit Document"><i class="fa-solid fa-pencil"></i></a>
                                            <a href="action.php?act=delete_committee_document&id=<?php echo $doc['id']; ?>" class="btn-action btn-delete" title="Delete Document" onclick="return confirm('Are you sure you want to delete this document? The file will be permanently deleted from the server.');" style="display: inline-flex;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        xhr.open('POST', 'action.php?act=save_committee_document&ajax=1', true);
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                try {
                    ...
                    window.location.href = 'committee_previous.php?success=' + (isEditMode ? 'updated' : 'created');
                }
            } else {
