<?php
$page_title = 'Manage Recent Activities';
...
try {
    $total_rows = $pdo->query("SELECT COUNT(*) FROM `recent_activities`")->fetchColumn();
    $total_pages = max(1, ceil($total_rows / $limit));
    
    $stmt = $pdo->prepare("SELECT * FROM `recent_activities` ORDER BY `created_at` DESC LIMIT :limit OFFSET :offset");
...
<div class="panel-card">
    <div class="panel-header">
        <h2 class="panel-title">Recent Activities Listing</h2>
        <a href="recent_activity_edit.php" class="btn-add"><i class="fa-solid fa-plus"></i> Add Activity</a>
    </div>
        ...
        <table class="dash-table">
            ...
            <tbody>
                <?php if (empty($list_items)): ?>
                    <tr><td colspan="5" class="no-data-row">No activities found. Click "Add Activity" to get started.</td></tr>
                <?php else: ?>
                        ...
                        <tr>
                            <td>
                                <?php if (!empty($item['image'])): ?>
                                    ...
                                    ?>
                                    <img src="<?php echo $img_src; ?>" class="thumbnail-img" alt="activity logo">
                                <?php else: ?>
                                    <div style="width: 60px; height: 40px; background-color: var(--sand); border-radius: 4px; display: flex; align-items: center; justify-content: center; font-size: 0.8rem; color: var(--gray);"><i class="fa-solid fa-image"></i></div>
                                <?php endif; ?>
                            </td>
                            <td style="font-weight: 700; min-width: 180px;"><?php echo htmlspecialchars($item['title']); ?></td>
                            <td style="color: var(--gray); font-size: 0.88rem; max-width: 320px; line-height: 1.5;"><?php echo htmlspecialchars(mb_strimwidth($item['description'], 0, 110, '...')); ?></td>
                            <td><?php echo date('d M Y', strtotime($item['created_at'])); ?></td>
                            <td>
                                <div class="action-cell">
                                    <a href="recent_activity_edit.php?id=<?php echo $item['id']; ?>" class="btn-action btn-edit" title="Edit Activity"><i class="fa-solid fa-pencil"></i></a>
                                    <a href="action.php?act=delete_recent_activity&id=<?php echo $item['id']; ?>" class="btn-action btn-delete" title="Delete Activity" onclick="return confirm('Are you sure you want to delete this activity?');"><i class="fa-solid fa-trash-can"></i></a>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
