...
try {
    $total_rows = $pdo->query("SELECT COUNT(*) FROM `blogs`")->fetchColumn();
...
<div class="panel-card">
    <div class="panel-header">
        <h2 class="panel-title">Blogs Listing</h2>
        <a href="blog_edit.php" class="btn-add"><i class="fa-solid fa-plus"></i> Add Blog Post</a>
    </div>
        ...
        <table class="dash-table">
            ...
            <tbody>
                <?php if (empty($list_items)): ?>
                    <tr><td colspan="6" class="no-data-row">No blog posts found. Click "Add Blog Post" to get started.</td></tr>
                <?php else: ?>
                        ...
                        <tr>
                            <td>
                                <?php if (!empty($item['image'])): ?>
                                    <?php
                                    $cover_src = htmlspecialchars($item['image']);
                                    if (strpos($item['image'], 'http') !== 0) {
                                        $cover_src = '../' . $cover_src;
                                    }
                                    ?>
                                    <img src="<?php echo $cover_src; ?>" class="thumbnail-img" alt="cover">
                                <?php else: ?>
                            ...
                            <td>
                                <div class="action-cell">
                                    <a href="blog_edit.php?id=<?php echo $item['id']; ?>" class="btn-action btn-edit" title="Edit Post"><i class="fa-solid fa-pencil"></i></a>
                                    <a href="action.php?act=delete_blog&id=<?php echo $item['id']; ?>" class="btn-action btn-delete" title="Delete Post" onclick="return confirm('Are you sure you want to delete this blog post?');"><i class="fa-solid fa-trash-can"></i></a>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
