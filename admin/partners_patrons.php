<?php
$page_title = 'Manage Patrons Documents';
...
try {
    $stmt_count = $pdo->prepare("SELECT COUNT(*) FROM `partner_documents` WHERE `doc_type` = ?");
    $stmt_count->execute([$doc_type]);
    $total_rows = $stmt_count->fetchColumn();
    $total_pages = max(1, ceil($total_rows / $limit));
    
    $stmt = $pdo->prepare("SELECT * FROM `partner_documents` WHERE `doc_type` = :doc_type ORDER BY `year` DESC, `created_at` DESC LIMIT :limit OFFSET :offset");
...
if ($edit_id > 0) {
    try {
        $stmt_edit = $pdo->prepare("SELECT * FROM `partner_documents` WHERE `id` = ? AND `doc_type` = ?");
    ...
    <div class="recent-card">
        <div class="recent-header">
            <h3 class="recent-title">
                ...
                <?php echo $edit_id > 0 ? 'Edit Document details' : 'Upload Patron PDF'; ?>
            </h3>
        </div>
        <div style="padding: 2rem;">
            <form id="document-form" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group" style="margin-bottom: 1.25rem;">
                    ...
                    <input type="text" id="doc_title" name="title" class="form-control" placeholder="e.g. Thanksgiving to Patrons List" value="<?php echo htmlspecialchars($edit_title); ?>" required>
                </div>
                ...
                <div style="display: flex; gap: 0.6rem;">
                    <?php if ($edit_id > 0): ?>
                        <a href="partners_patrons.php" class="btn btn-cancel" style="flex: 1; text-align: center; text-decoration: none; display: flex; align-items: center; justify-content: center; height: 42px; border-radius: 6px;">Cancel</a>
                    <?php endif; ?>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center; padding: 1rem 1.5rem;">
                                        <div style="display: inline-flex; gap: 0.4rem;">
                                            <a href="partners_patrons.php?edit=<?php echo $doc['id']; ?>&page=<?php echo $page; ?>" class="btn-action btn-edit" title="Edit Document"><i class="fa-solid fa-pencil"></i></a>
                                            <a href="action.php?act=delete_partner_document&id=<?php echo $doc['id']; ?>" class="btn-action btn-delete" title="Delete Document" onclick="return confirm('Are you sure you want to delete this document? The file will be permanently deleted from the server.');" style="display: inline-flex;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleFileSelect(file, isDrag) {
        ...
        if (isDrag) {
            // Set the files property of fileInput for drag and drop only
    ...
    form.addEventListener('submit', function(e) {
        ...
        xhr.open('POST', 'action.php?act=save_partner_document&ajax=1', true);
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                try {
                    ...
                    window.location.href = 'partners_patrons.php?success=' + (isEditMode ? 'updated' : 'created');
                }
            } else {
