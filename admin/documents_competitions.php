<?php
$page_title = 'Manage Competitions & Winners';
...
try {
    $stmt_count = $pdo->prepare("SELECT COUNT(*) FROM `association_documents` WHERE `doc_type` = ?");
    $stmt_count->execute([$doc_type]);
    $total_rows = $stmt_count->fetchColumn();
    $total_pages = max(1, ceil($total_rows / $limit));
    
    $stmt = $pdo->prepare("SELECT * FROM `association_documents` WHERE `doc_type` = :doc_type ORDER BY `year` DESC, `created_at` DESC LIMIT :limit OFFSET :offset");
...
if ($edit_id > 0) {
    try {
        $stmt_edit = $pdo->prepare("SELECT * FROM `association_documents` WHERE `id` = ? AND `doc_type` = ?");
    ...
    <div class="recent-card">
        <div class="recent-header">
            <h3 class="recent-title">
                ...
                <?php echo $edit_id > 0 ? 'Edit Document details' : 'Upload Competition PDF'; ?>
            </h3>
        </div>
        <div style="padding: 2rem;">
            <form id="document-form" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group" style="margin-bottom: 1.25rem;">
                    ...
                    <input type="text" id="doc_title" name="title" class="form-control" placeholder="e.g. Art Competition Winners 2026" value="<?php echo htmlspecialchars($edit_title); ?>" required>
                </div>
                ...
                <div style="display: flex; gap: 0.6rem;">
                    <?php if ($edit_id > 0): ?>
                        <a href="documents_competitions.php" class="btn btn-cancel" style="flex: 1; text-align: center; text-decoration: none; display: flex; align-items: center; justify-content: center; height: 42px; border-radius: 6px;">Cancel</a>
                    <?php endif; ?>
                            ...
                            <?php foreach ($documents as $doc): ?>
                                <tr>
                                    <td style="padding: 1rem 1.5rem;">
                                        ...
                                        <div style="font-size: 0.8rem; color: var(--gray); font-weight: 600;">Year: <span class="category-badge" style="font-size: 0.72rem; padding: 0.15rem 0.4rem;"><?php echo htmlspecialchars($doc['year']); ?></span></div>
                                    </td>
                                    <td style="text-align: center; padding: 1rem 1.5rem;">
                                        ...
                                        <?php else: ?>
                                            <span style="font-size: 0.8rem; color: var(--gray); font-style: italic;">No File</span>
                                        <?php endif; ?>
                                    </td>
                                    <td style="text-align: center; padding: 1rem 1.5rem;">
                                        <div style="display: inline-flex; gap: 0.4rem;">
                                            <a href="documents_competitions.php?edit=<?php echo $doc['id']; ?>&page=<?php echo $page; ?>" class="btn-action btn-edit" title="Edit Document"><i class="fa-solid fa-pencil"></i></a>
                                            <a href="action.php?act=delete_association_document&id=<?php echo $doc['id']; ?>" class="btn-action btn-delete" title="Delete Document" onclick="return confirm('Are you sure you want to delete this document? The file will be permanently deleted from the server.');" style="display: inline-flex;">
        ...
        <?php if ($total_pages > 1): ?>
            <div class="pagination-container" style="display: flex; justify-content: center; align-items: center; gap: 0.5rem; padding: 1.5rem 2rem; border-top: 1px solid var(--border); background-color: var(--white); flex-wrap: wrap;">
                ...
                <?php if ($page < $total_pages): ?>
                    <a href="?page=<?php echo $page + 1; ?><?php echo $edit_id > 0 ? '&edit=' . $edit_id : ''; ?>" class="btn-pagination" style="text-decoration: none; padding: 0.5rem 0.85rem; border: 1px solid var(--border); border-radius: 6px; color: var(--gray); font-size: 0.85rem; font-weight: 700; transition: var(--transition);">Next <i class="fa-solid fa-angle-right"></i></a>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fileInput.addEventListener('change', () => {
        if (fileInput.files.length > 0) {
            handleFileSelect(fileInput.files[0], false);
        }
    });

    function handleFileSelect(file, isDrag) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        xhr.open('POST', 'action.php?act=save_association_document&ajax=1', true);
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                try {
                    ...
                    window.location.href = 'documents_competitions.php?success=' + (isEditMode ? 'updated' : 'created');
                }
            } else {
