<?php
$page_title = 'Manage Membership Requests';
...
try {
    $total_rows = $pdo->query("SELECT COUNT(*) FROM `membership_requests`")->fetchColumn();
    $total_pages = max(1, ceil($total_rows / $limit));
    
    $stmt = $pdo->prepare("SELECT * FROM `membership_requests` ORDER BY `created_at` DESC LIMIT :limit OFFSET :offset");
...
<?php if (!empty($success)): ?>
    <div class="alert-success">
        ...
        <span><?php 
            if ($success === 'deleted') echo 'Membership request deleted successfully!';
...
<div class="panel-card">
    <div class="panel-header">
        <h2 class="panel-title">Membership Requests Log</h2>
    </div>
    ...
    <div class="table-responsive">
        <table class="dash-table">
            <thead>
                <tr>
                    <th style="padding: 1rem 1.5rem;">Applicant Details</th>
                    <th style="padding: 1rem 1.5rem;">Residential Address</th>
                    <th style="padding: 1rem 1.5rem;">Message / Reference</th>
            ...
            <tbody>
                <?php if (empty($requests)): ?>
                    <tr>
                        <td colspan="5" class="no-data-row" style="padding: 2.5rem 1.5rem !important;">No membership requests found.</td>
                    </tr>
                <?php else: ?>
                        ...
                        <tr>
                            <td style="padding: 1rem 1.5rem; vertical-align: top;">
                                ...
                                <div style="font-size: 0.8rem; color: var(--gray); margin-bottom: 0.15rem;">
                                    <i class="fa-solid fa-envelope" style="width: 16px;"></i> <?php echo htmlspecialchars($req['email']); ?>
                                </div>
                                <div style="font-size: 0.8rem; color: var(--gray);">
                                    <i class="fa-solid fa-phone" style="width: 16px;"></i> <?php echo htmlspecialchars($req['phone']); ?>
                                </div>
                            </td>
                            <td style="padding: 1rem 1.5rem; vertical-align: top; font-size: 0.88rem; line-height: 1.5; color: var(--dark); max-width: 250px; word-wrap: break-word; white-space: normal;">
                                <?php echo nl2br(htmlspecialchars($req['address'])); ?>
                            </td>
                            <td style="padding: 1rem 1.5rem; vertical-align: top; font-size: 0.88rem; line-height: 1.5; color: var(--gray); max-width: 300px; word-wrap: break-word; white-space: normal;">
                                ...
                                if (empty($req['message'])) {
                                    echo '<span style="font-style: italic; color: #bbb;">None</span>';
                                } else {
                                    $msg = $req['message'];
                                    $max_len = 80;
                                    if (strlen($msg) > $max_len) {
                                        $truncated = substr($msg, 0, $max_len);
                                        $last_space = strrpos($truncated, ' ');
                                        if ($last_space !== false) {
                                            $truncated = substr($truncated, 0, $last_space);
                                        }
                                        echo htmlspecialchars($truncated) . '... ';
                                        echo '<button type="button" class="btn-read-more" data-full-message="' . htmlspecialchars($msg) . '" style="background: none; border: none; color: var(--red); font-weight: 700; cursor: pointer; font-size: 0.82rem; padding: 0; text-decoration: underline; display: inline;">Read More</button>';
                                    } else {
                            ...
                            <td style="padding: 1rem 1.5rem; text-align: center; vertical-align: top; font-size: 0.82rem; font-weight: 600; color: var(--gray);">
                                ...
                                <span style="font-size: 0.75rem; font-weight: normal; opacity: 0.8;"><?php echo date('h:i A', strtotime($req['created_at'])); ?></span>
                            </td>
                            <td style="padding: 1rem 1.5rem; text-align: center; vertical-align: top;">
                                <a href="action.php?act=delete_membership_request&id=<?php echo $req['id']; ?>" class="btn-action btn-delete" title="Delete Request" onclick="return confirm('Are you sure you want to delete this membership request? This action cannot be undone.');" style="display: inline-flex;">
